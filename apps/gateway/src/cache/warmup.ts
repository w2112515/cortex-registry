...
import type { EnrichedService, ServiceId, ReputationData, ServiceState } from '../types.js';
...
export async function executeWarmup(
  ...
  try {
    ...
    if (cfg.mode === 'lazy') {
      ...
      const exists = await redis.exists(CACHE_CONFIG.serviceList!.key as string);
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error('Warmup timeout')), cfg.timeout)
    );
 ...
 */
async function performFullWarmup(config: WarmupConfig): Promise<{
  services: EnrichedService[];
  reputations: Map<ServiceId, ReputationData>;
}> {
  const redis = getRedisClient();
  const services: EnrichedService[] = [];
  const reputations = new Map<ServiceId, ReputationData>();

  // Check if we have existing cached data
  const cachedList = await redis.get(CACHE_CONFIG.serviceList!.key as string);
  ...
  if (cachedList) {
    ...
    await redis.setex(
      CACHE_CONFIG.serviceList!.key as string,
      CACHE_CONFIG.serviceList!.ttl,
      JSON.stringify([])
    );
  }
...
export function getWarmupMetrics(): {
  ...
  cached: { services: number; reputations: number } | null;
} {
